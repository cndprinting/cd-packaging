...
const SYSTEM = `You turn a sales rep's own notes on one lead into the single next task for that rep.
...
- basis = the exact note fragment (max 140 chars) the task comes from, copied verbatim.

Reply with JSON only: {"kind": "...", "task": "...", "basis": "..."}`;

export async function computeNextTask(input: { companyName: string; stage: string | null; followUpAt: Date | null; followUpNote: string | null; notes: { body: string; authorName: string; createdAt: Date }[] }): Promise<NextTask | null> {
  const claude = getClaude();
  if (!claude) return null;
  const human = input.notes.filter((n) => n.body.trim()).slice(0, 8);
  if (human.length === 0) return { kind: "none", task: "", basis: "" };
  const user = [
    ...
    ...human.map((n) => `[${n.createdAt.toISOString().slice(0, 10)} ${n.authorName}] ${n.body.replace(/\s+/g, " ").slice(0, 700)}`),
  ].join("\n");
  try {
...
export async function refreshNextTask(prisma: any, leadId: string): Promise<NextTask | null> {
  const lead = await prisma.lead.findUnique({
    where: { id: leadId },
    select: { companyName: true, stage: true, followUpAt: true, followUpNote: true, notes: { where: { kind: "human" }, orderBy: { createdAt: "desc" }, take: 8, select: { body: true, authorName: true, createdAt: true } } },
  });
  if (!lead) return null;
