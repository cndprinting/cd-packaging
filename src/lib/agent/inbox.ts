...
import { agentSend, agentMarySend, agentCustomerSend, OWNERS, MARY, SHAYLA, onMaryQuote, kickoffMailerCity, onMailerCityReply, isAutoReply, isWeekendET, firstName } from "@/lib/agent/agent";
...
[Agent] Customer acknowledged the decline graciously ("${txt.slice(0, 80)}") - conversation closed, no action needed.`.slice(0, 8000) } }); } catch { /* ignore */ }
    ...
    try {
      ...
      if (claude) {
        ...
        // Benjy 9/24 ("we say Toothily ... why not say his name Anthony"): the
        // prompt never told the model the person's name, so it greeted the company.
        const first = firstName(lead.contactName);
        const u = `The person's first name is ${first || "(unknown)"}; the company is ${lead.companyName}. Open with "Hi ${first || "there"}," and never greet the company by name.\nThey replied to our quote for ${lead.productName}.\nOur quote was:\n${lead.agentQuote || "(not recorded)"}\n\nTheir message:\n${fullText}`;
        ...
        // Belt and braces: a greeting aimed at the company becomes the first name.
        if (draft && first) {
          const co = lead.companyName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          draft = draft.replace(new RegExp(`^(\\s*(?:<p>)?\\s*(?:Hi|Hello|Hey|Dear)\\s+)${co}(\\s*[,.!])`, "i"), `$1${first}$2`);
        }
      }
