                ...
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <div className="flex gap-1">
                    ...
                    {/* Internal-only actions — hidden on wholesale rows so the
                        ...
                        via the generic Send/Reject). */}
                    {q.status === "draft" && !(q as any).sourcingVendor && <Button variant="ghost" size="sm" className="gap-1 text-blue-600" onClick={async () => { await fetch("/api/quotes", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: q.id, status: "sent" }) }).catch(() => {}); setQuotes(p => p.map(x => x.id === q.id ? { ...x, status: "sent" } : x)); }}><Send className="h-3.5 w-3.5" />Send</Button>}
                    {(q.status === "draft" || q.status === "sent") && !(q as any).sourcingVendor && <Button variant="ghost" size="sm" className="gap-1 text-gray-600" onClick={() => alert("Send to CSR/Salesperson — email integration coming soon")}><Mail className="h-3.5 w-3.5" /></Button>}
                    {q.status === "approved" && !(q as any).sourcingVendor && <Button variant="ghost" size="sm" className="gap-1 text-purple-600" onClick={async () => { await fetch("/api/quotes", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: q.id, status: "converted" }) }).catch(() => {}); setQuotes(p => p.map(x => x.id === q.id ? { ...x, status: "converted" } : x)); }}><Package className="h-3.5 w-3.5" />Convert</Button>}
                    <Button variant="ghost" size="sm" className="gap-1" onClick={() => window.open(`/dashboard/quotes/${q.id}/print`, '_blank')} title="Customer quote letter"><Printer className="h-3.5 w-3.5" /></Button>
                    {!(q as any).sourcingVendor && <Button variant="ghost" size="sm" className="gap-1 text-purple-600" onClick={() => window.open(`/dashboard/quotes/${q.id}/detailed`, '_blank')} title="Internal detailed estimate breakdown">📊</Button>}
                    {q.status !== "archived" ? (
